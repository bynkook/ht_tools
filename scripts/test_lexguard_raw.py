...
목적:
  - 모든 tool의 성공/실패/관찰 케이스별 실제 raw_result 구조 확인
  - build_legal_context_system_prompt()의 transport 실패 판별 조건 확정
  - FabriX의 tool_result_to_dict()가 받는 structuredContent 구조를 실물 검증

주의:
  - 이 스크립트는 raw 응답 구조 관찰이 목적이며 semantic PASS/FAIL을 강하게 판정하지 않는다.
  - 특히 law_article_tool은 최신 시행본 기준으로 law.go.kr가 조문 본문을 반환하지 않는 케이스가 있어,
    응답 수신과 조문 본문 성공을 동일시하면 안 된다.
...
class Case:
    ...
    tool: str
    args: dict
    expect_success: bool | None  # True=정상 응답 기대, False=실패 응답 기대, None=관찰
...
CASES: list[Case] = [
    ...
    # ── law_article_tool ─────────────────────────────────────────────────────
    Case(
        label="law_article_tool / 관찰: 근로기준법 제50조 (본문 미반환 가능)",
        tool="law_article_tool",
        args={"law_name": "근로기준법", "article_number": "50"},
        expect_success=None,
    ),
    Case(
        label="law_article_tool / 관찰: 법령명만 (개요 반환 여부 확인)",
        tool="law_article_tool",
        args={"law_name": "민법"},
        expect_success=None,
    ),
...
async def run_case(client: httpx.AsyncClient, case: Case, summary: Summary) -> None:
    if case.expect_success is True:
        expect_tag = "SUCCESS 기대"
    elif case.expect_success is False:
        expect_tag = "FAILURE 유도"
    else:
        expect_tag = "관찰"
