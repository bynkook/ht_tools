...
class TestModeChatRuntime(BaseChatRuntime):
    ...
    async def _event_generator(
        ...
        *,
        ...
        try:
            for provider_id in self._iter_discovery_providers(decision, selected_provider):
                ...
                for payload in self._serialize_guarded_event(
                    ...
                    yield payload
                capabilities = await self.mcp_host.discover_provider_capabilities(provider_id=provider_id)
                for payload in self._serialize_guarded_event(
                    guard,
            ...
            if not plans:
                for payload in self._serialize_guarded_event(
                    ...
                    recorder.tool_decision(
                        ...
                        raw={"route": decision.route, "contents": runtime_input.contents[-1]},
            ...
            for index, plan in enumerate(plans, start=1):
                current_plan = plan
                plan_event_details = self._build_plan_event_details(plan, selection_rank=index)
                for payload in self._serialize_guarded_event(
                    guard,
                    recorder.tool_decision(
            ...
            for payload in self._serialize_guarded_event(
                guard,
                recorder.context_aggregated(raw=context_summary, event_details=recorder_event_details),
            ):
                yield payload
            for payload in self._serialize_guarded_event(
                guard,
            ...
            error_provider = current_plan.provider if current_plan is not None else recorder.provider
    ...
    def _build_context_summary(self, base_system_prompt: str | None, context_results: list[dict[str, Any]]) -> dict[str, Any]:
        return build_context_debug_payload(
            base_system_prompt=base_system_prompt,
            context_results=context_results,
        )

    def _select_primary_provider(self, decision: PlannerDecision, selected_provider: str | None = None) -> str:
        if decision.plans:
            return decision.plans[0].provider
        return selected_provider or self.mcp_host.default_provider_id

    def _iter_discovery_providers(self, decision: PlannerDecision, selected_provider: str | None = None) -> tuple[str, ...]:
        ordered_providers = [plan.provider for plan in decision.plans] or [selected_provider or self.mcp_host.default_provider_id]
        seen: set[str] = set()
        resolved: list[str] = []
    ...
    def _build_plan_event_details(self, plan: ToolPlan, *, selection_rank: int) -> dict[str, Any]:
        candidate_summary = list(plan.provenance.provider_candidates) if plan.provenance.provider_candidates else None
        return {
            **self._build_provider_event_details(plan.provider),
            "selection_reason": plan.provenance.reason or plan.provenance.decision_source,
    ...
    def _build_error_event_details(
        ...
        *,
        ...
        return {
            **self._build_plan_event_details(current_plan, selection_rank=completed_count + 1),
            "partial_failure": {
